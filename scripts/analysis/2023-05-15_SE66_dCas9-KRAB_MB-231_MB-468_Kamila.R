...
data <- "data/raw/2023-02-28_231_468_KB_SE66_from_Kamila.xls"

# Name the experiment
experiment <- "SE66_dCas9-KRAB_MB-231_MB-468"
...
qpcr_data <- read_excel(data, sheet = "Results", skip = 44, col_names = TRUE) |> 
  slice(1:(n()-5)) |> 
  filter(!`Sample Name` %in% c("NTC", "NT")) |> 
  filter(!`Sample Name` %in% c("RT-", "NoRT")) |> 
  separate(`Well Position`, into = c("row", "column"), sep = 1, convert = TRUE) |> 
  rename(primer = `Target Name`)

# Filter based on cell line used (231 or 468)
cell_line <- "468"

qpcr_data <- qpcr_data |> 
  filter(str_detect(`Sample Name`, paste0("^", cell_line))) |> #filters to only cell line of interest samples
  mutate(`Sample Name` = str_sub(`Sample Name`, 5, nchar(`Sample Name`))) #remove the first 4 characters from sample name to get rid of cell line specification
...
# Remove bad replicates Kamila identified (231 Rep 1, 468 rep 4)
qpcr_data <- qpcr_data |> 
  filter(!grepl("SCR-4", `Sample Name`, ignore.case = TRUE)) #remove bad rep (based on HKG Ct values)
...
for (i in 1:length(test_list)){
  ...
  write_tsv(results_exp, paste(tables_path, "/", names(test_list)[i], "-", experiment, "_fold_change_results_", cell_line, "_cells.txt", sep = ""))
  ...
  write_tsv(as.data.frame(tt_pval), paste(tables_path, "/", names(test_list)[i], "-", experiment, "_t-test_p-values_BH_correction_", cell_line, "_cells.txt", sep = ""))
  ...
  write_tsv(as.data.frame(dtpvs), paste(tables_path, "/", names(test_list)[i], "-", experiment, "_Dunnetts_test_results_", cell_line, "_cells.txt", sep = ""))
  ...
  write_tsv(as.data.frame(tuk_results), paste(tables_path, "/", names(test_list)[i], "-", experiment, "_tukey_test_results_", cell_line, "_cells.txt", sep = ""))
  ...
  p1 <- ggplot(final_data, aes(x = sample, y = rel_conc)) +
    ...
    theme_Publication() +
    scale_fill_manual(values = c("SCR" = "gray30",
                                 "SE66" = "red")) +
    scale_color_manual(values = c("SCR" = "gray30",
                                  "SE66" = "red")) +
    theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1))
  
  ggsave(paste(plots_path, "/", names(test_list)[i], "-", experiment, "_", cell_line, "_cells.png", sep = ""), plot = p1, width = 2, height = 4, units = "in", dpi = 600)
  ggsave(paste(plots_path, "/", names(test_list)[i], "-", experiment, "_", cell_line, "_cells.pdf", sep = ""), plot = p1, width = 2, height = 4, units = "in", dpi = 600)
}
