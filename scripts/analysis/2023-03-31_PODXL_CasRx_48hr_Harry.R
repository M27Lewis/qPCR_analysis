...
data <- "data/raw/2023-03-28_134254_First_qPCR_from_second_time_48h_transfection_PODXL_KD.xls"

# Name the experiment
experiment <- "PODXL_CasRx_48hr_Harry"
...
qpcr_data <- read_excel(data, sheet = "Results", skip = 44, col_names = TRUE) |> 
  slice(1:(n()-5)) |> 
  dplyr::filter(!`Sample Name` %in% c("NTC", "No Template", "NT", "Blank")) |> 
  dplyr::filter(!`Sample Name` %in% c("-RT", "RT-", "Non RT")) |>   
